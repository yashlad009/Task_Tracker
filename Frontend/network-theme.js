(() => {
  ...
  const NODE_COUNT = 56, LINK_DISTANCE = 210, CURSOR_INFLUENCE = 165;
  ...
  function seed() { nodes.length = 0; for (let i = 0; i < NODE_COUNT; i += 1) nodes.push({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, vx: (Math.random() - 0.5) * 0.2, vy: (Math.random() - 0.5) * 0.2, r: Math.random() * 1.4 + 1.0 }); }
  ...
  function frame(time = 0) { ctx.clearRect(0, 0, canvas.width, canvas.height); document.body.classList.add('network-theme-active'); drawAura(time);
